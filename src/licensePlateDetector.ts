...
const loadImageFromFile = async (filePath: string): Promise<Buffer> => {
...
interface Detection {
  boxes: number[][];  // [y1, x1, y2, x2] format
  scores: number[];
  classes: number[];
}

// Interface for license plate region
...
async function preprocessImage(imagePath: string): Promise<tf.Tensor3D> {
  ...
  const resizedImage = tf.image.resizeBilinear(decodedImage as tf.Tensor3D, [300, 300]);
...
function detectEdges(imageTensor: tf.Tensor3D): tf.Tensor3D {
  ...
  const expandedImage = adjustedContrast.expandDims(0) as unknown as tf.Tensor4D;
  ...
  const edgesX = tf.conv2d(blurred, expandedX as unknown as tf.Tensor4D, 1, 'same');
  const edgesY = tf.conv2d(blurred, expandedY as unknown as tf.Tensor4D, 1, 'same');

  // Combine the edges and enhance
  const edgesMagnitude = tf.sqrt(
    tf.add(tf.square(edgesX), tf.square(edgesY))
  );
  ...
  // Return the edges
  return normalizedEdges.squeeze() as tf.Tensor3D;
}

// Function to directly save debug images for development
async function saveDebugImage(tensor: tf.Tensor3D, name: string): Promise<void> {
  ...
  if (tensor.shape[2] === 1) {
    const rgb = tf.tile(tensor, [1, 1, 3]) as tf.Tensor3D;
    imageData = await tf.node.encodePng(rgb.mul(tf.scalar(255)).cast('int32') as tf.Tensor3D);
    rgb.dispose();
  } else {
    imageData = await tf.node.encodePng(tensor.mul(tf.scalar(255)).cast('int32') as tf.Tensor3D);
  }
...
async function findPlateRegions(imagePath: string): Promise<PlateRegion[]> {
  try {
    console.log('Starting findPlateRegions for image:', imagePath);
    // Load image
    const imageBuffer = await loadImageFromFile(imagePath);
    const image = tf.node.decodeImage(imageBuffer) as tf.Tensor3D;
    ...
    for (const threshold of thresholds) {
      ...
      await saveDebugImage(thresholded as unknown as tf.Tensor3D, `threshold_${threshold}`);
      ...
      const edgesData = await thresholded.array() as unknown as boolean[][];
      const plateRegions: PlateRegion[] = [];
        ...
        for (let y = 0; y < height - windowHeight; y += stepY) {
          for (let x = 0; x < width - windowWidth; x += stepX) {
            ...
            for (let wy = y; wy < windowEndY; wy += sampleRate) {
              if (!edgesData[wy]) continue; // Skip if row doesn't exist
...
async function cropImage(imagePath: string, region: PlateRegion): Promise<Buffer> {
  try {
...
export async function detectLicensePlate(imagePath: string): Promise<Buffer | null> {
  try {
    console.log('-----------------------------------');
    ...
    if (model) {
      ...
      // Run inference
      const result = await model.executeAsync(inputTensor) as tf.Tensor[];
      ...
      if (plateRegions.length === 0) {
        ...
        const fallbackRegion: PlateRegion = {
